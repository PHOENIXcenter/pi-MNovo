"""Training and validation orchestration for the pi-MNovo backbone."""
...
from .. import utils
from .data import DeNovoDataModule
...
def _maybe_reuse_index(
    index_path: str, filenames: Optional[List[str]]
) -> Optional[List[str]]:
...
class MetricsCsvCallback(pl.callbacks.Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if trainer.optimizers:
            self.last_lr = float(
                trainer.optimizers[0].param_groups[0].get("lr", float("nan"))
            )

    def on_before_optimizer_step(
        self, trainer, pl_module, optimizer, optimizer_idx=None
    ):
        total_norm = 0.0
...
def train(
    ...
    if train_is_not_index:
        train_index_path = [
            _stable_index_path(tmp_dir.name, "Train", train_filenames, config, True)
        ]
    ...
    if val_is_not_index:
        val_index_path = [
            _stable_index_path(tmp_dir.name, "valid", val_filenames, config, True)
        ]
    ...
    # Derive scheduler lengths from the sampled epoch budget.
    device_count = max(torch.cuda.device_count(), 1)
    batches_per_device = int(
        len(train_dataloader) / (device_count * config["accumulate_grad_batches"])
    )
    ...
    if config["train_from_scratch"]:
        ...
        if not model_filename:
            raise ValueError("Provide --checkpoint unless train_from_scratch is true.")
    ...
    return sorted(
        {
            fn
            for fn in files
            if os.path.isfile(fn) and os.path.splitext(fn.lower())[1] in supported_ext
        }
    )
