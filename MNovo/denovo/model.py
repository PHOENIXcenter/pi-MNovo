    ...
    **kwargs : Dict
    ...
    def forward(
        ...
        def worker (logits, mass, i , output_logits):
            ...
            if self.PMC_enable == False:
                ...
                sequence = list(filter((self.decoder.get_pad_idx()).__ne__, top_tokens_beam[i]))
                token_true = [self.decoder._idx2aa[each] for each in sequence]
                pred_mass, _sequence = mass_cal("".join(token_true))
    ...
    def _apply_freezing(self):
        ...
        for name, param in self.named_parameters():
            param.requires_grad = not self._matches_any_prefix(name, self.freeze_module_prefixes)
    ...
    def configure_optimizers(
        ...
        if self.finetune_strategy == "layerwise":
            ...
            optimizer = torch.optim.AdamW([p for p in self.parameters() if p.requires_grad], **self.opt_kwargs)
