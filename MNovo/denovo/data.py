...
class DeNovoDataModule(pl.LightningDataModule):
    """Build training, validation, test, and prediction data loaders."""
    ...
    def setup(self, stage=None):
        """Attach existing LMDB indexes to datasets on each worker process."""
    ...
    def prepare_data(self) -> None:
        """Create missing LMDB indexes once before distributed setup."""
        LOGGER.info("Preparing spectrum indexes")

        def build_or_reuse(index_path, filenames):
            if filenames is not None and os.path.exists(index_path):
                LOGGER.info("Reusing spectrum index: %s", index_path)
                filenames = None
            lock = filenames is not None
            LmdbSpectrumIndex(
                index_path,
                filenames,
                self.ms_level,
        ...
        if self.train_index is None and self.mode == "fit":
            for each in self.train_index_path:
                build_or_reuse(each, self.train_filenames)
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        requested = self.train_num_samples or 500_000
        sample_count = min(int(requested), len(self.train_dataset))
